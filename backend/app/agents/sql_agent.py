...
class SQLAgent(BaseAgent):
    ...
    async def _process_sql_style_query(self, df: pd.DataFrame, sql_query: str) -> Dict[str, Any]:
        ...
        print("🔍 SQL Agent: Processing SQL-style query: " + sql_query)
        print(f"🔍 SQL Agent: DataFrame shape: {df.shape}, columns: {list(df.columns)}")
        ...
        try:
            ...
            if "sum(" in query_lower and "group by" in query_lower:
                ...
                print(f"🔍 SQL Agent: Extracted group_col: {group_col}, sum_col: {sum_col}")
                
                if group_col and sum_col and group_col in df.columns and sum_col in df.columns:
                    print(f"🔍 SQL Agent: Processing groupby with {group_col} and {sum_col}")
                    
                    # Add internal timeout mechanism using executor
                    def process_groupby():
                    ...
                    with ThreadPoolExecutor() as executor:
                        result_future = asyncio.get_event_loop().run_in_executor(executor, process_groupby)
                        try:
                            result = await asyncio.wait_for(result_future, timeout=30)  # 30 sec timeout
                            
                            execution_time = time.time() - start_time
                            print(f"🔍 SQL Agent: Query completed in {execution_time:.2f} seconds")
                            ...
                            print("❌ SQL Agent: Query execution timed out after 30 seconds")
                            raise TimeoutError("SQL query execution timed out")
            ...
            return {
                ...
                "summary": "Showing first 20 rows of data",
